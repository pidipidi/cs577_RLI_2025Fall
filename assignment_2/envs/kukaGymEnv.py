...
class KukaGymEnv(gymnasium.Env):
  ...
  def reset(self, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None):
    """ Reset KukaGymEnv """
    # We must call super reset for seeding to work
  ...
  def close(self):
    try:
      # Check if the connection is active before trying to disconnect
      if p.isConnected():
        p.disconnect()
    except p.error as e:
      # This catches the 'Not connected' error if it somehow still happens
      # but is better than relying on __del__
      print(f"PyBullet disconnection error during close: {e}")    
  ...
  def _reward(self):
    ...
    # DEFAULT: Add a sparse reward (i.e., lifting the grasped block)    
    #if (): 
    #  reward 
