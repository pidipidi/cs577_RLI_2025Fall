...
from vi_sol import valueIterAgent
...
def make_reward(env, goal_state):
    ...
    def reward_fn(state):
        ...
        if env.unwrapped.isValid(state, check_collision=True) is False: return -1
...
if __name__ == '__main__':
    ...
    env.unwrapped.set_start_state(start)
    env.unwrapped.set_goal_size(1.0)
    env.unwrapped.set_goal_state(goal)
    env.unwrapped.set_objects(obstacles)
    env.unwrapped.set_robot_size(robot_size)    
    env.unwrapped.set_reward(make_reward(env, goal))
    env.reset()
